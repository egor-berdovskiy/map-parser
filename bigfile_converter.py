...
async def main():
    print('Async big files converter!')
    ...
    for i_file, file in enumerate(directory):
        ...
        sliced_df = np.array_split(df, 4)

        results_slices = []
        for chunk in sliced_df:
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
                tasks = []
                for column, data in chunk['coordinates (lat, lon)'].items():
                    lat, lon = data.split(' ')
                ...
                results_slices.append(results)

        result = [item for sublist in results_slices for item in sublist]

        # SORT
        addresses = sorted(result, key=lambda d: list(d.keys())[0])
